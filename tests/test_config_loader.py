...
from core.config_loader import load_config, _get_default_config
...
class TestLoadConfig:
    ...
    def test_falls_back_to_example_config(self, temp_dir):
        ...
        example_config = {"context_root": "~/example", "work_path_patterns": ["~/work/"]}
        (config_dir / "example-config.json").write_text(json.dumps(example_config))
...
class TestDefaultConfig:
    ...
    def test_default_llm_config(self):
        """Default LLM config uses haiku model."""
        config = _get_default_config()

        assert config['llm_config']['model'] == 'haiku'
        assert config['llm_config']['max_tokens'] == 150
